...
frontend/
  ...
  src/app/
    ...
    post/
      [id]/
        page.tsx                full-screen detail page; header: format badge, attribution, cover image (Books), title, author, interest tags; SectionRenderer renders all 15 section types in order; sticky comment bar (safe-area aware) with always-visible round btn-primary send + like/save (lamp gold when active) + share; slide-up animation, swipe-right-to-close; attribution: Submitted by @user (blue verified badge) for user content; Deepscroll + violet badge for seed/official; comment add/delete writes the new count through to the cached feed lists; design-exploration dispatch by designForFormat(post.format): DetailBack/DetailHeader/DetailBar/CommentsList slots, baseline when null; loading/not-found always baseline
    components/
      FeedHeader.tsx             feed tab bar extracted from page.tsx (search button + scrollable tab strip + sliding indicator markup); scroll-sync refs and listeners stay in page.tsx; indicator half-width read from offsetWidth (no hardcoded size)
      redesign/types.ts          design-variant contract: PostCardViewProps (stopPropagation pre-wrapped handlers), CommentsSheetViewProps, FeedStateProps, NavItem/FeedNavProps, Detail*Props, HeaderConfig, FeedSlots/DetailSlots/DesignModule (all slots nullable; null = baseline)
      redesign/registry.ts       designModule(design) lookup merging per-design feed.ts + detail.ts slot modules
      redesign/{folio,console,stage}/feed.ts|detail.ts  per-design slot wiring (feed.ts = feed slots, detail.ts = comments + detail chrome slots)
      redesign/console/          Console (people+concepts): ConsoleCard (edge-to-edge dossier panel, rotated rail label, numbered teasers, hairline data strip, cell action toolbar), ConsoleFeedStates (terminal loading, framed empty), ConsoleBottomNav (squared labeled dock, inverted steel-blue active), ConsoleHeader (indexed folder tabs, inverted active chip, square "/" search slot), ConsoleCommentsSheet (right-side inspector panel, log entries, "&gt;" prompt input), ConsoleDetailChrome (dossier cover-sheet header, squared back, full-width toolbar bar + log-style comments list)
      redesign/stage/            Stage (questions+stories): StageCard (borderless frosted slab, floating metadata pills, detached capsule action cluster), StageFeedStates (pulsing skeleton slabs, slab empty), StageBottomNav (floating frosted pill dock), stageHeader (floating capsule strip, width-tracking pill indicator, frosted search circle), StageCommentsSheet (detached floating sheet, baseline drag gestures, chat-bubble rows via StageCommentRow), StageCommentsList, StageDetailChrome (frosted slab header, floating circle back, floating pill detail bar)
      redesign/folio/            Folio (books+facts): FolioCard (no-card print page, small-caps text action row), FolioFeedStates (ellipsis loading, hairline empty), FolioBottomNav (hairline bar, underline tick), folioHeader (running-head tabs, hidden indicator, bare magnifier), FolioCommentsSheet (full-screen Notes page, slides from right), FolioEndnotes + FolioCommentsList (hanging-indent endnotes), FolioDetailChrome (title-page header, bare back arrow, hairline detail bar)
      ...
      BottomNav.tsx              fixed bottom nav: Chat / Stats / Feed (flame) / Create (plus-circle) / Profile; items built as NavItem[] (NAV_ICONS export = inner svg content); optional design prop (feed page only) dispatches to a redesign BottomNav variant, default null = baseline render; active item glows via double drop-shadow filter (lamp color); bar has upward radial glow (boxShadow); safe-area-inset-bottom aware; prefetches the sibling routes on mount so the first tap skips the route-chunk download (incl. the ~517 KB recharts chunk for /stats)
    ...
    lib/
      ...
      useComments.ts            comment list state hook (fetch/post/delete, loadedRef count guard) shared by CommentsBottomSheet and redesign comment variants
      useBodyDesign.ts          mirrors the active design onto body[data-design] (texture variants + portal cascade); effect-only, no hydration mismatch
