...
mobile/                         React Native app (Expo SDK 56, TypeScript, expo-router, NativeWind 4 + tailwindcss 3.4); phase 5 = profiles, follows, search and stats in Stage
  ...
  src/components/BottomNav.tsx  Stage floating dock port: frosted pill (h-56) inset left/right 16 and 12px above the safe-area bottom; active prop ("feed"|"stats"|"profile") = neutral white/12% circle, no glow; Stats -> /stats, Feed -> /, Profile -> own public profile or /login; Chat/Create -> "Coming soon" toast
