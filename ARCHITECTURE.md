...
frontend/
  ...
  src/app/
    ...
    components/
      ...
      redesign/{folio,console,stage}/feed.ts|detail.ts  per-design slot wiring (start as all-null stubs; filled by each design build)
      ...
      redesign/stage/            Stage (questions+stories) feed: StageCard (borderless frosted slab, floating metadata pills, detached capsule action cluster), StageFeedStates (pulsing skeleton slabs, slab empty), StageBottomNav (floating frosted pill dock), stageHeader (floating capsule strip, width-tracking pill indicator, frosted search circle)
