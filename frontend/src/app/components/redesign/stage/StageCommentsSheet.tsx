...
// on all corners. Keeps the baseline drag gestures (swipe up expands, swipe
// down collapses or closes, live translateY feedback); the spring-in
// keyframes live in detail.css with a reduced-motion guard.
...
export default function StageCommentsSheet({ postId, onClose, onCountChange }: CommentsSheetViewProps) {
  const { user } = useAuth()
  ...
  // Drag handle: swipe up expands to 75vh, swipe down collapses or closes
  useEffect(() => {
  ...
  return createPortal(
    // Portals escape the page wrapper, so the design scope is set here.
    <div className="fixed inset-0 z-50" data-design="stage" onClick={onClose}>
      {/* Backdrop */}
