        ...
        {content.map((item, i) => (
          <div key={i} className="flex flex-col gap-2">
            <div className="flex items-start gap-2">
              ...
              <span className="text-sm text-ink-muted line-through leading-relaxed">{unescapeDollar(item.myth)}</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-good text-xs mt-0.5 shrink-0 font-bold">✓</span>
              <span className="prose-post">{unescapeDollar(item.reality)}</span>
            </div>
          </div>
        ))}
      </div>
